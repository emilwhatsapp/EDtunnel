...
function getวเลสConfig(userIDs, hostName) {
	const commonUrlPart = `:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2Fbodongcf#PORT 443`;
	const commonUrlPartt = `:80?encryption=none&security=none&type=ws&host=${hostName}&path=%2Fbodongcf#PORT 443`;
	const hashSeparator = "################################################################";
	...
	const output = userIDArray.map((userID) => {
		...
		const วเลสSec = atob(pt) + '://' + userID + atob(at) + พร็อกซีไอพี + commonUrlPartt;
		return `<center><h2>LIFETIME SAMPAI PROXY MATI</h2>${hashSeparator}\nPort 443
---------------------------------------------------------------
${วเลสMain}
<button onclick='copyToClipboard("${วเลสMain}")'><i class="fa fa-clipboard"></i> Copy Port 443</button>
---------------------------------------------------------------
Port 80
---------------------------------------------------------------
${วเลสSec}
<button onclick='copyToClipboard("${วเลสSec}")'><i class="fa fa-clipboard"></i> Copy Port 80</button>
---------------------------------------------------------------`;
	}).join('\n');
